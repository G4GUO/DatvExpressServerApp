...
int express_find( void )
{
    ...
    {
        loggerf("Cannot claim USB interface %d\n",ret);
...
*/
    ...
    {
        loggerf("Express cannot set interface alt %d\n",ret);
...
int express_firmware_load(const char *fx2_filename)
{
    ...
    {
        loggerf("Express cannot find firmware file %s\n",fx2_filename);
        return EXP_IHX;
    }
    return EXP_OK;
}
...
int express_fpga_load( const char *fpga_filename )
{
    ...
    {
        loggerf("Express HW cannot find FPGA file %s\n",fpga_filename);
        return EXP_RBF;
    }
    return EXP_OK;
}
